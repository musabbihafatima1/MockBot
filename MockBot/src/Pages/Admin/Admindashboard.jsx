...
function AdminDashboard() {
  ...
  const [loadingRejections, setLoadingRejections] = useState([]);
  ...
  const acceptCompany = async (id) => {
    try {
      setLoadingApprovals(prev => [...prev, id]);
      const response = await axios.post(`http://localhost:5000/api/admin/approve-request/${id}`);
      const approvedCompany = response.data;
      ...
      setLoadingApprovals(prev => prev.filter(item => item !== id));
    }
  };
  ...
  const rejectCompany = async (id) => {
    try {
      setLoadingRejections(prev => [...prev, id]);
      await axios.post(`http://localhost:5000/api/admin/reject-request/${id}`);
      setNewRequests(prev => prev.filter(c => c._id !== id));
      setShowDetails(false);
      ...
      setLoadingRejections(prev => prev.filter(item => item !== id));
    }
  };
                        ...
                        <div key={company._id} style={{ ...styles.companyCard, ...styles.pendingCompanyCard }}>
                          ...
                          <div style={styles.actionButtons}>
                            ...
                            <button 
                              style={styles.approveBtn} 
                              onClick={() => acceptCompany(company._id)}
                              disabled={loadingApprovals.includes(company._id)}
                            >
                              {loadingApprovals.includes(company._id) ? (
                            ...
                            <button 
                              style={styles.rejectBtn} 
                              onClick={() => rejectCompany(company._id)}
                              disabled={loadingRejections.includes(company._id)}
                            >
                              {loadingRejections.includes(company._id) ? (
                                <FaSpinner style={styles.spinner} />
                              ) : (
                                <>
                                  <FaTimes /> Reject
                        ...
                        {selectedCompany.status === 'pending' && (
                          <>
                            <button 
                              ...
                              onClick={() => acceptCompany(selectedCompany._id)}
                              disabled={loadingApprovals.includes(selectedCompany._id)}
                            >
                              {loadingApprovals.includes(selectedCompany._id) ? (
                                <FaSpinner style={styles.spinner} />
                              ) : (
                                <>
                                  <FaCheck /> Approve
                            ...
                            <button 
                              ...
                              onClick={() => rejectCompany(selectedCompany._id)}
                              disabled={loadingRejections.includes(selectedCompany._id)}
                            >
                              {loadingRejections.includes(selectedCompany._id) ? (
                                <FaSpinner style={styles.spinner} />
                              ) : (
                                <>
                                  <FaTimes /> Reject
